...
const BASE_URL =
  import.meta.env.VITE_API_BASE_URL || "https://thermos-rho.vercel.app";
...
async function request<T>(
  ...
  try {
    ...
    throw new Error(
      "Can't reach the THERMOS backend. Please try again shortly."
    );
  }
  ...
  if (!res.ok || !json.success) {
    ...
    if (code === "FORTYGUARD_UNAVAILABLE") {
      throw new Error(
        "Heat intelligence data is temporarily unavailable. Please try again shortly."
      );
    }
...
export const api = {
  ...
  getForecastHeat: (assetId: string, hoursAhead = 6) =>
    request<HeatReading>(
      `/heat/forecast/${assetId}?hours_ahead=${hoursAhead}`
    ),

  getRisk: (assetId: string) =>
    request<RiskScore>(`/risk/${assetId}`),

  getAllRiskAndPriority: () =>
    request<RiskAndPriorityResponse>("/risk"),
  ...
  resetFacility: () =>
    request<FacilitySelectResult>("/facility/reset", {
      method: "POST",
    }),

  getFacilityHeatmap: () =>
    request<FacilityHeatmap>("/heat/facility-map"),
};
